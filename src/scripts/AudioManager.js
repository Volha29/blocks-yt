...
export default class AudioManager {
    ...
    startMusic(key) {
        ...
        console.log(`25 AM isAudioYTPlay =${this.isAudioYTPlay}, startMusic`); 
        this.playMusic();
    }
    playMusic() {
        if (!this.bgMusic || !this.game) return;
        if (!this.game.sdk.isAudioYTPlay || !this.isMusicPlay) {
            console.log(`30 AM isAudioYTPlay =${this.isAudioYTPlay}, isPause`); 
            this.bgMusic.pause();
            return;
        }
        if (this.bgMusic.isPaused) {
            console.log(`33 AM isAudioYTPlay =${this.isAudioYTPlay}, isResume`); 
            this.bgMusic.resume();
        } else if (!this.bgMusic.isPlaying) {
            console.log(`33 AM isAudioYTPlay =${this.isAudioYTPlay}, isPlay`); 
            this.bgMusic.play();
        }
